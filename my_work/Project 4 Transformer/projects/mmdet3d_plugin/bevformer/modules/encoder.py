...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    def get_reference_points(H, W, Z=8, num_points_in_pillar=4, dim='3d', bs=1, device='cuda', dtype=torch.float):
        ...
        if dim == '3d':
            ...
            # Generate normalized coordinates for reference points
            ...
            # Stack coordinates to create 3D reference points
            # (np, H, W, 3) -> (np, 3, H, W) -> (np, 3, H * W) -> (np, H * W, 3)
            ref_3d = torch.stack((xs, ys, zs), -1)
            ref_3d = ref_3d.permute(0, 3, 1, 2).flatten(2).permute(0, 2, 1)

            # Repeat the reference points across the batch dimension
    ...
    def point_sampling(self, reference_points, pc_range,  img_metas):
        ...
        # (batch, np, bh * bw, 4) -> (np, batch, bh * bw, 4)
        reference_points = reference_points.permute(1, 0, 2, 3)
        D, B, num_query = reference_points.size()[:3]
        num_cam = lidar2img.size(1)

        # (np, batch, bh * bw, 4) -> (np, batch, num_cam, bh * bw, 4, 1)
        ...
        # lidar2img is the transformation matrix from LiDAR coordinates to camera image coordinates using lidar2img.
        # Perform matrix multiplication of lidar2img with reference_points, converting them to float32
        # squeeze the last dimension

        # (np, batch, num_cam, bh * bw, 4)
        ...
        # Extract the x and y coordinates from the projected 3D reference points
        # Ensure non-zero depth to avoid division by zero, using eps
        # Normalize the x and y coordinates by the depth values
